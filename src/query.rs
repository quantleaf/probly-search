 ...
 */
pub fn query<T: Eq + Hash + Clone + Debug>(
    index: &mut Index<T>,
    fields_boost: &Vec<f64>,
    bm25k1: f64,
    bm25b: f64,
    tokenizer: Tokenizer,
    filter: Filter,
    ...
    for term_pre_filter in terms {
        ...
        if term != "" {
            ...
            for j in 0..expanded_terms.len() {
                let e_term = &expanded_terms[j];
                let expansion_boost = {
                    if e_term == &term {
                        1 as f64
                    } else {
                ...
                if let Some(term_node) = term_node_option {
                    if let Some(term_node_option_first_doc) = &term_node.borrow().first_doc {
                        ...
                        if document_frequency > 0 {
                            ...
                            let idf = f64::ln(
                                1_f64
                                    + ((&docs.len() - document_frequency) as f64 + (0.5 as f64))
                                        / (document_frequency as f64 + 0.5 as f64),
                            );
        ...
        fn it_should_return_doc_1() {
            ...
            for doc in docs {
                add_document_to_index(
                    &mut idx,
                    &vec![title_extract, text_extract],
                    tokenizer,
                    filter,
                    doc.id,
                    doc,
